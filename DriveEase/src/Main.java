...
public class Main {
    ...
    public void processRental(Customer customer,Car car,SecurityProtocol protocol1, LoyaltyProcessor processor, List<CostFactor> addOns, int days){
        ...
        if(!protocol1.checkAccess(customer, car)){
            ...
            double baseRentalPriceAfter = baseRentalPriceBefore;

            for (CostFactor costFactor : addOns) {
                baseRentalPriceAfter += costFactor.calculate(days);
                services.append(costFactor.getServiceDetail()).append(" ");
            }
            ...
            System.out.println("+ Service:" +services +"| Cost for this additional insurance for " + days + " days: " + (baseRentalPriceAfter - baseRentalPriceBefore));
